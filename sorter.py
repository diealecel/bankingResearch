...
print "Welcome to Diego's Indexer v2!\nFile processing is now starting.\n"
...
for variateChapter in xrange(ENDING_CHAPTER - STARTING_CHAPTER + 1): # This creates [0, 1]
    for variateLetter in CHAPTER_LETTERS:
        for variateYear in variateYearRange: # This creates [0, 1, ..., 20, 'prelim']
            ...
            bankLawsSite = "http://uscode.house.gov/view.xhtml?hl=false&edition=" + variateYear + "&req=granuleid%3AUSC-prelim-title12-chapter" + str(STARTING_CHAPTER + variateChapter) + variateLetter
            ...
            with open(specimenName, 'w') as post:
                    ...
                    for line in source:
                        while True:
                            ...
                            # This implements data into the objects.
                                ...
                                undesirables.add(str(STARTING_CHAPTER + variateChapter) + ' ' + variateYear + '::' + potentialTag)
                                invalidTag += 1
                            if not neededRepeat:
                                break
                            else:
                ...
                if numberOfObjects > 0 and not triedRetry:
                    ...
                    status = 'FAILED'
                print '{0:5}{1:6}{2:9}{3:10}{4:10}{5}'.format(str(STARTING_CHAPTER + variateChapter) + variateLetter, 'CURR' if variateYear == 'prelim' else variateYear, str(numberOfObjects), str(round(validTag / float(validTag + invalidTag) * 100, 4)) + '%', str(round(time.time() - timeStart, 4)), status)
